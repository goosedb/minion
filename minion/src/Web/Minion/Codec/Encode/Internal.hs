module Web.Minion.Codec.Encode.Internal where
...
-- | How to encode a value of type `a` into a lazy 'Bytes.Lazy.ByteString' with Content-Type = `ct`
class Encode ct a where
  encode :: a -> Bytes.Lazy.ByteString

-- | How to encode a value of type `a` into `Wai.StreamingBody` with Content-Type = `ct`
...
{- | How to encode a value of type `a` into 'Wai.Response' with ContentType = second `encodeBody` argument
